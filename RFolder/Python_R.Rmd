---
title: "Linear Regression and XGBoost"
...
merged = pd.read_csv("/Users/robbiefeldstein/Documents/Programming/Election2024/RFolder/merged.csv")
```

```{python}
training_df = merged[["Days Since 01-01-23", "biden"]].copy()
training_df["biden"] = training_df["biden"].astype(float)
X = training_df.dropna()
y = merged["Biden (D)"]
y.dropna(inplace=True)
X["SinFreq1"] = np.sin(2 * np.pi * X["Days Since 01-01-23"] / 365)
X["CosFreq1"] = np.cos(2 * np.pi * X["Days Since 01-01-23"] / 365)
X.head()
```
```{python}
