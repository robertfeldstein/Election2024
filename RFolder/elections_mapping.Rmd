...
model_for_date <- function(df, date){
  ...
  m <- fit_model(y, locs, X, "matern_isotropic", silent = TRUE, m_seq = 50)
  return(m)
}
...
make_prediction_for_date <- function(df, date){
  ...
  pred <- predictions(m, locs_pred = locs_pred, X_pred = X_pred, m = 50)
  return(pred)
}
...
```{r}
#Overall error
mean(sqrt(errors$error), na.rm = TRUE)
```
